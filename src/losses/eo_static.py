...
def equalized_odds_loss(label_pred, sensitive, label_true):
    eps   = 1e-10
    # keep only rows where the sensitive attribute is not missing
    ...
    # split sensitive groups into two complementary groups
    s_bar = sensitive
    s = 1.0 - sensitive
    ...
    # compute number of negative and positive samples per group
    ...
    return torch.nan_to_num(eq_odds)
