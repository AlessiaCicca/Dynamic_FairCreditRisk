...
NOTE: aligned to run_realData.py — the dynamic model is a discrete-time
survival landmark model. After CV we collapse per-bin hazards into
PD(L, L+horizon) via S = prod(1 - h), then evaluate threshold / AUC /
separation at the (subject, landmark) level.
"""
...
def collapse_to_pd12(oof_hazard, event_bin, ids, lmk_vals, n_bins,
    ...
    S(L, L+h) = prod_j (1 - h_j)  ->  PD = 1 - S
    y12       = 1 se default in QUALCHE bin della finestra
    Con complete_only=True si tengono solo le finestre osservate per intero
    (n_bin osservati == n_bins), cioe' i veri PD-(L, L+horizon).
    """
    ...
    y12  = g["ev"].max().rename("y12")    # default in QUALCHE bin = default in (L, L+h]
    cnt  = g.size().rename("n")
    out  = pd.concat([pd12, y12, cnt], axis=1).reset_index()
    if complete_only:                      # solo finestre osservate per intero
...
def main():
    ...
    dynamic_data = build_dynamic(
        ...
        horizon=cfg["horizon"], delta=cfg.get("delta", 1),   # 3 // 1 = 3 bin per finestra
        id_col=ID_COL, time_col=TIME_COL,
        first_event_col="FirstEventTime",
        sens_col="sens_loan", enc_cat=enc_cat,
    )
    ...
    # --- COLLAPSE hazard per-bin -> PD-(L, L+horizon) (come nel reale) ---
    ...
    # soglia / AUC / Brier sulla PD-12 (non sugli hazard di bin)
    ...
    # Summary (coerente con run_realData.py)
    ...
    df_agg, df_dyn_lmk, df_auc = run_fairness_analysis(
        ...
        sens_dynamic=dyn_sens_collapsed,    # era dynamic_data["sensitive"]
    ...
    if args.grid_search:
        df_grid = run_grid_search(
            ...
            n_folds=cfg["n_folds"],
            eo_mode_d=cfg["eo_mode_d"],
            schedule_mode_d=cfg["schedule_mode_d"],   # <-- coerenza con il run principale
