...
# Input columns
PERF_COLS_FULL = ["loan_sequence_number","monthly_reporting_period","current_upb","current_loan_delinquency_status","loan_age","remaining_months_to_maturity",
    "defect_settlement_date","modifications_flag","zero_balance_code","zero_balance_effective_date","current_interest_rate","current_deferred_upb",
    "due_date_last_paid_installment","mi_recoveries","net_sales_proceeds","non_mi_recoveries","expenses","legal_costs","maintenance_costs","taxes_and_insurance",
    "miscellaneous_expenses","actual_loss","modification_cost","step_modification_flag","deferred_payment_plan","estimated_ltv","zero_balance_removal_upb",
    "delinquent_accrued_interest","delinquency_due_to_disaster","borrower_assistance_status","current_month_modification_loss","interest_bearing_upb"]

# Output columns
PERF_COLS_KEEP = ["loan_sequence_number", "monthly_reporting_period","current_upb","current_loan_delinquency_status","loan_age","remaining_months_to_maturity",
    "modifications_flag","zero_balance_code","zero_balance_effective_date","current_interest_rate","current_deferred_upb","estimated_ltv","delinquency_due_to_disaster",
    "borrower_assistance_status"]


# Extract performance files from drive
def extract_performance_zips(year, freddie_dir, perf_local):
    ...
    if not extracted:
        print(f"No TIME files extracted for {year}.")
    return extracted
...
def load_performance(perf_txt_files, loan_ids, parquet_path, chunk_size=200000):
    writer = None
    for f in perf_txt_files:
        fname = os.path.basename(f)
        with open(f, "r") as fh:
            n_cols_file = len(fh.readline().split("|"))
        for chunk in pd.read_csv(f, sep="|", header=None, names=PERF_COLS_FULL[:n_cols_file],dtype=str, low_memory=False,chunksize=chunk_size, on_bad_lines="skip"):
...
# Create some time columns that could be useful to the analysis of the dataset
def add_time_columns(perf):
    ...
    numeric_columns=["current_upb",  "loan_age",  "remaining_months_to_maturity","current_interest_rate", "estimated_ltv", "current_deferred_upb"]
    ...
    status = perf["current_loan_delinquency_status"].str.strip()
    # Create MAP
    is_current = status.isin({"", "0", "00"})
    
    perf["_is_default"] = is_current.map({True: 0,False: 1})
    

    # FirstEventTime definition
    default_loans = perf[perf["_is_default"] == 1]
    grouped = default_loans.groupby("loan_sequence_number")
    first_event = grouped["loan_age"].min()
    
    # Series rename
    first_event = first_event.rename("FirstEventTime")
...
def build_panel(perf, matched):
    ...
    if dups > 0:
        print(f"Duplicate rows on (loan, month).")
    return panel

# Order columns and save
def save_panel(panel, matched, output_path):
    id_cols   = ["loan_sequence_number", "period_quarter", "period_year","quarter", "monthly_reporting_period"]
    perf_cols = ["loan_age", "remaining_months_to_maturity", "current_upb", "current_interest_rate", "estimated_ltv", "current_deferred_upb", "current_loan_delinquency_status",
                 "modifications_flag", "zero_balance_code","zero_balance_effective_date","delinquency_due_to_disaster", "borrower_assistance_status"]
    target_cols=[]
